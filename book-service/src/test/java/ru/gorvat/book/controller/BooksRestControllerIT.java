...
class BooksRestControllerIT {
    ...
    void findBooks_ReturnsBooksList () throws Exception {
        ...
        this.mockMvc.perform (requestBuilder)
                ...
                .andDo (print ()).andExpectAll (status ().isOk (), content ().contentTypeCompatibleWith (MediaType.APPLICATION_JSON), content ().json ("""
                        [
    ...
    void findBooks_UserIsNotAuthorized_ReturnsForbidden () throws Exception {
        var requestBuilder = MockMvcRequestBuilders.get ("/book-api/books").param ("filter", "книга").with (jwt ());

        this.mockMvc.perform (requestBuilder).andDo (print ()).andExpectAll (status ().isForbidden ());
    }
    ...
    void createBook_RequestIsInvalid_ReturnsProblemDetail () throws Exception {
        var requestBuilder = MockMvcRequestBuilders.post ("/book-api/books").contentType (MediaType.APPLICATION_JSON).content ("""
                ...
                """).locale (Locale.of ("ru", "RU")).with (jwt ().jwt (builder -> builder.claim ("scope", "edit_catalogue")));

        this.mockMvc.perform (requestBuilder)

                .andDo (print ()).andExpectAll (status ().isBadRequest (), content ().contentTypeCompatibleWith (MediaType.APPLICATION_PROBLEM_JSON), content ().json ("""
                        {
                        "errors" : [
    ...
    void createBook_UserIsNotAuthorized_ReturnsForbidden () throws Exception {
        var requestBuilder = MockMvcRequestBuilders.post ("/book-api/books").contentType (MediaType.APPLICATION_JSON).content ("""
                {"title" : " ", "author" : null, "publication" : 1}
                """).locale (Locale.of ("ru", "RU")).with (jwt ().jwt (builder ->
